...
function AuthScreen({ onSuccess }) {
  ...
  const [siShowPwd, setSiShow]  = useState(false);
  const [siErrs, setSiErrs]     = useState({});

  const [suName, setSuName]     = useState("");
  const [suEmail, setSuEmail]   = useState("");
  const [suPwd, setSuPwd]       = useState("");
  const [suConfirm, setSuConf]  = useState("");
  const [suShowPwd, setSuShow]  = useState(false);
  ...
  const handleSignIn = async () => {
    const errs = {};
    if (!isEmail(siEmail)) errs.email = "Please enter a valid email.";
    if (!siPwd)            errs.pwd   = "Password is required.";
    if (Object.keys(errs).length) { setSiErrs(errs); return; }
    setSiErrs({});
    setLoading(true);
    try {
      const res  = await fetch("/api/auth", {
        ...
        body: JSON.stringify({ email: siEmail.trim(), password: siPwd })
      });
      const data = await res.json();
      if (data.ok) {
        sessionStorage.setItem("dc_access_code", data.token || siEmail.trim());
        onSuccess();
      } else {
        setSiErrs({ pwd: data.error || "Incorrect email or password." });
      }
    } catch {
      setSiErrs({ pwd: "Could not reach server." });
    }
    setLoading(false);
  };
  ...
  const handleSignUp = async () => {
    const errs = {};
    if (!suName.trim())          errs.name    = "Please enter your name.";
    if (!isEmail(suEmail))       errs.email   = "Please enter a valid email.";
    if (suPwd.length < 8)        errs.pwd     = "Password must be at least 8 characters.";
    if (suPwd !== suConfirm)     errs.confirm = "Passwords do not match.";
    if (Object.keys(errs).length) { setSuErrs(errs); return; }
    setSuErrs({});
    setLoading(true);
    try {
      const res  = await fetch("/api/register", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: suName.trim(), email: suEmail.trim(), password: suPwd })
      });
      const data = await res.json();
      if (data.ok) {
        sessionStorage.setItem("dc_access_code", data.token || suEmail.trim());
        onSuccess();
      } else {
        setSuErrs({ email: data.error || "Could not create account." });
      }
    } catch {
      setSuErrs({ email: "Could not reach server." });
    }
    setLoading(false);
  };

  const inp = (extra={}) => ({
    background:"var(--gl2)", border:"1px solid var(--b2)", color:"var(--tx)",
    padding:"11px 14px 11px 38px", borderRadius:10, fontSize:14,
    fontFamily:"'Sora',sans-serif", outline:"none", width:"100%",
    transition:"border-color .2s, box-shadow .2s", ...extra
  });
  const inpFocus = { borderColor:"rgba(0,212,184,0.45)", boxShadow:"0 0 0 3px rgba(0,212,184,0.10)" };
  const errStyle = { fontSize:12, color:"var(--rd)", marginTop:5 };
  const labelStyle = { fontSize:10.5, fontWeight:600, color:"var(--tx3)", textTransform:"uppercase",
    letterSpacing:"0.06em", marginBottom:7, display:"block" };
  const iconWrap = { position:"relative" };
  const iconSvg  = { position:"absolute", left:12, top:"50%", transform:"translateY(-50%)",
    color:"var(--tx3)", pointerEvents:"none", width:16, height:16 };
  ...
  const strengthColors = ["#ff6b6b","#ffb347","#e8c97a","#00d4b8"];
  const strengthLabels = ["Weak","Fair","Good","Strong"];

  return (
    <div style={{position:"relative",zIndex:1,minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"80px 16px 32px"}}>

      {/* Logo */}
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:36}}>
        <div style={{width:40,height:40,background:"linear-gradient(135deg,var(--tl),var(--tl2))",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:"0 0 18px rgba(0,212,184,0.35)"}}>🩺</div>
      ...
      {/* Card */}
      <div style={{width:"100%",maxWidth:420,background:"linear-gradient(160deg,var(--n2) 0%,rgba(10,22,40,0.9) 100%)",border:"1px solid rgba(0,212,184,0.14)",borderRadius:20,padding:"36px 32px 32px",boxShadow:"0 24px 64px rgba(0,0,0,0.45)",backdropFilter:"blur(12px)"}}>

        {/* Tabs */}
        <div style={{display:"flex",background:"var(--n3)",borderRadius:10,padding:4,marginBottom:28,gap:2}}>
          {[["signin","Sign In"],["signup","Create Account"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)} style={{flex:1,padding:"9px 0",border:"none",outline:"none",borderRadius:7,fontFamily:"'Sora',sans-serif",fontSize:14,fontWeight:500,cursor:"pointer",transition:"all .22s",
              background:tab===id?"linear-gradient(135deg,var(--tl),var(--tl2))":"transparent",
              color:tab===id?"var(--n)":"var(--tx2)",
              boxShadow:tab===id?"0 2px 10px rgba(0,212,184,0.3)":"none"
            }}>{label}</button>
          ))}
        </div>
        ...
        {tab === "signin" && (
          <div style={{animation:"fu .3s ease both"}}>
            <div style={{fontSize:22,fontWeight:600,marginBottom:6,letterSpacing:"-0.3px"}}>Welcome back</div>
            <div style={{fontSize:13,color:"var(--tx2)",marginBottom:24,lineHeight:1.5}}>Sign in to access your prescriptions</div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={iconWrap}>
                <input type="email" placeholder="you@example.com" value={siEmail}
                  onChange={e=>{setSiEmail(e.target.value);setSiErrs(p=>({...p,email:null}));}}
                  onKeyDown={e=>e.key==="Enter"&&!loading&&handleSignIn()}
                  style={{...inp(), ...(siErrs.email?{borderColor:"var(--rd)"}:{})}}
                />
                <svg style={iconSvg} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25H4.5A2.25 2.25 0 012.25 17.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5H4.5a2.25 2.25 0 00-2.25 2.25m19.5 0L12 13.5 2.25 6.75"/></svg>
              </div>
              {siErrs.email && <div style={errStyle}>{siErrs.email}</div>}
            </div>
            ...
            <div style={{marginBottom:16}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:7}}>
                ...
                <button onClick={()=>showToastMsg(isEmail(siEmail)?"✓ Reset link sent to "+siEmail:"Enter your email above first", isEmail(siEmail)?"success":"error")}
                  style={{fontSize:12,color:"var(--tl)",fontWeight:500,cursor:"pointer",background:"none",border:"none",fontFamily:"'Sora',sans-serif",padding:0}}>
                  Forgot password?
                </button>
              </div>
              <div style={iconWrap}>
                <input type={siShowPwd?"text":"password"} placeholder="••••••••" value={siPwd}
                  onChange={e=>{setSiPwd(e.target.value);setSiErrs(p=>({...p,pwd:null}));}}
                  onKeyDown={e=>e.key==="Enter"&&!loading&&handleSignIn()}
                  style={{...inp({paddingRight:40}), ...(siErrs.pwd?{borderColor:"var(--rd)"}:{})}}
                />
                <svg style={iconSvg} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75A4.5 4.5 0 0012 2.25v0a4.5 4.5 0 00-4.5 4.5v3.75m-2.25.75h13.5A1.5 1.5 0 0120.25 12.75v7.5A1.5 1.5 0 0118.75 21.75H5.25A1.5 1.5 0 013.75 20.25v-7.5A1.5 1.5 0 015.25 10.5z"/></svg>
                <button onClick={()=>setSiShow(v=>!v)} type="button"
                  style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"var(--tx3)",display:"flex",alignItems:"center",padding:4}}>
                  <EyeIcon show={siShowPwd}/>
                </button>
              </div>
              {siErrs.pwd && <div style={errStyle}>{siErrs.pwd}</div>}
            </div>

            <button onClick={handleSignIn} disabled={loading}
              style={{width:"100%",padding:13,background:loading?"rgba(0,212,184,0.4)":"linear-gradient(135deg,var(--tl),var(--tl2))",border:"none",borderRadius:10,fontFamily:"'Sora',sans-serif",fontSize:15,fontWeight:600,color:"var(--n)",cursor:loading?"not-allowed":"pointer",boxShadow:"0 4px 18px rgba(0,212,184,0.28)",marginTop:4,display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"opacity .2s"}}>
              {loading
                ? <><span style={{width:16,height:16,border:"2px solid var(--n)",borderTop:"2px solid transparent",borderRadius:"50%",animation:"spin 1s linear infinite",display:"inline-block"}}/>Signing in…</>
                : <>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6A2.25 2.25 0 005.25 5.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M18 12H9m0 0l3-3m-3 3l3 3"/></svg>
                    Sign In
                  </>
              }
            </button>
        ...
        {tab === "signup" && (
          <div style={{animation:"fu .3s ease both"}}>
            <div style={{fontSize:22,fontWeight:600,marginBottom:6,letterSpacing:"-0.3px"}}>Create your account</div>
            <div style={{fontSize:13,color:"var(--tx2)",marginBottom:24,lineHeight:1.5}}>Understand your prescriptions clearly, instantly</div>

            {/* Name */}
            <div style={{marginBottom:16}}>
              <label style={labelStyle}>Full Name</label>
              <div style={iconWrap}>
                <input type="text" placeholder="Rajesh Kumar" value={suName}
                  onChange={e=>{setSuName(e.target.value);setSuErrs(p=>({...p,name:null}));}}
                  style={{...inp(), ...(suErrs.name?{borderColor:"var(--rd)"}:{})}}
                />
              ...
              {suErrs.name && <div style={errStyle}>{suErrs.name}</div>}
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={iconWrap}>
                <input type="email" placeholder="you@example.com" value={suEmail}
                  onChange={e=>{setSuEmail(e.target.value);setSuErrs(p=>({...p,email:null}));}}
                  style={{...inp(), ...(suErrs.email?{borderColor:"var(--rd)"}:{})}}
                />
                <svg style={iconSvg} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25H4.5A2.25 2.25 0 012.25 17.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5H4.5a2.25 2.25 0 00-2.25 2.25m19.5 0L12 13.5 2.25 6.75"/></svg>
              </div>
              {suErrs.email && <div style={errStyle}>{suErrs.email}</div>}
            </div>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={iconWrap}>
                <input type={suShowPwd?"text":"password"} placeholder="Min. 8 characters" value={suPwd}
                  onChange={e=>{setSuPwd(e.target.value);setSuErrs(p=>({...p,pwd:null}));setStrength(calcStrength(e.target.value));}}
                  style={{...inp({paddingRight:40}), ...(suErrs.pwd?{borderColor:"var(--rd)"}:{})}}
                />
                <svg style={iconSvg} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75A4.5 4.5 0 0012 2.25v0a4.5 4.5 0 00-4.5 4.5v3.75m-2.25.75h13.5A1.5 1.5 0 0120.25 12.75v7.5A1.5 1.5 0 0118.75 21.75H5.25A1.5 1.5 0 013.75 20.25v-7.5A1.5 1.5 0 015.25 10.5z"/></svg>
                <button onClick={()=>setSuShow(v=>!v)} type="button"
                  style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"var(--tx3)",display:"flex",alignItems:"center",padding:4}}>
                  <EyeIcon show={suShowPwd}/>
                </button>
              </div>
              {suErrs.pwd && <div style={errStyle}>{suErrs.pwd}</div>}
              {suPwd && (
                <div style={{display:"flex",gap:4,alignItems:"center",marginTop:8}}>
                  ...
                  <span style={{fontSize:11,marginLeft:6,color:strength>0?strengthColors[strength-1]:"var(--tx3)",whiteSpace:"nowrap",transition:"color .3s"}}>
            ...
            <div style={{marginBottom:16}}>
              ...
              <div style={iconWrap}>
                <input type="password" placeholder="Re-enter password" value={suConfirm}
                  onChange={e=>{setSuConf(e.target.value);setSuErrs(p=>({...p,confirm:null}));}}
                  style={{...inp(), ...(suErrs.confirm?{borderColor:"var(--rd)"}:{})}}
                />
                <svg style={iconSvg} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75A4.5 4.5 0 0012 2.25v0a4.5 4.5 0 00-4.5 4.5v3.75m-2.25.75h13.5A1.5 1.5 0 0120.25 12.75v7.5A1.5 1.5 0 0118.75 21.75H5.25A1.5 1.5 0 013.75 20.25v-7.5A1.5 1.5 0 015.25 10.5z"/></svg>
              </div>
              {suErrs.confirm && <div style={errStyle}>{suErrs.confirm}</div>}
            </div>

            <button onClick={handleSignUp} disabled={loading}
              style={{width:"100%",padding:13,background:loading?"rgba(0,212,184,0.4)":"linear-gradient(135deg,var(--tl),var(--tl2))",border:"none",borderRadius:10,fontFamily:"'Sora',sans-serif",fontSize:15,fontWeight:600,color:"var(--n)",cursor:loading?"not-allowed":"pointer",boxShadow:"0 4px 18px rgba(0,212,184,0.28)",marginTop:4,display:"flex",alignItems:"center",justifyContent:"center",gap:8,transition:"opacity .2s"}}>
              {loading
                ? <><span style={{width:16,height:16,border:"2px solid var(--n)",borderTop:"2px solid transparent",borderRadius:"50%",animation:"spin 1s linear infinite",display:"inline-block"}}/>Creating account…</>
                : <>
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zM4 19.235v-.11a6.375 6.375 0 0112.75 0v.109A12.318 12.318 0 0110.374 21c-2.331 0-4.512-.645-6.374-1.766z"/></svg>
                    Create Account
                  </>
              }
            </button>

            <div style={{fontSize:11.5,color:"var(--tx3)",textAlign:"center",marginTop:16,lineHeight:1.5}}>
              By creating an account you agree to our{" "}
              <span style={{color:"var(--tl)",cursor:"pointer"}}>Terms of Service</span>{" "}and{" "}
              <span style={{color:"var(--tl)",cursor:"pointer"}}>Privacy Policy</span>.
            </div>
          </div>
        )}
      </div>
      ...
      {/* Inline toast */}
      {toastMsg && (
        <div style={{position:"fixed",bottom:28,left:"50%",transform:"translateX(-50%)",background:"var(--n2)",border:`1px solid ${toastType==="error"?"rgba(255,107,107,0.4)":"rgba(0,212,184,0.25)"}`,color:toastType==="error"?"var(--rd)":"var(--tx)",fontSize:13.5,fontWeight:500,padding:"12px 22px",borderRadius:40,boxShadow:"0 8px 28px rgba(0,0,0,0.4)",animation:"fu .35s ease",whiteSpace:"nowrap",zIndex:999}}>
          {toastMsg}
        </div>
      )}
    </div>
